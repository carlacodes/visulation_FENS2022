...
fname='confusion_matrix_eclair_PS.mat'
info_heatmap = rd.loadmat(bin_folder + os.sep + fname)
fname_raw='confusion_matrix_raw_eclair_PS.mat'
...
for i in range(0, len(heatmap_bychan)):
    ...
    plt.title('Eclair Roved F0, Site Number '+(np.array2string(channel_list[i])))
    plt.savefig(bin_folder + '\seabornheatmap_15062022_ECLAIR_PS_chan'+np.array2string(channel_list[i])+'.png', dpi=500, bbox_inches='tight')
