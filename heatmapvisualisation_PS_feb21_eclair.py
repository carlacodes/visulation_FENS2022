...
for i in range(0, len(heatmap_bychan)):
    ...
    plt.title('Eclair Roved F0 Intra-trial, Site Number '+(np.array2string(channel_list[i])))
    plt.savefig(bin_folder + '\seabornheatmap_0407022_ECLAIR_feb21_PS_chan'+np.array2string(channel_list[i])+'.png', dpi=500, bbox_inches='tight')
