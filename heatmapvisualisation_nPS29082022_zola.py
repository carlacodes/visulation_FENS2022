...
fname='confusion_matrix_zola_nPS.mat'
info_heatmap = rd.loadmat(bin_folder + os.sep + fname)
fname_raw='confusion_matrix_zola_raw_nPS.mat'
...
# str={'craft', 'in contrast to','when a', 'accurate','rev. target', 'of science', 'pink noise', 'instruments'}

str = ['craft', 'in contrast to', 'when a', 'accurate', 'rev. target', 'of science', 'pink noise',
       'instruments']
...
for i in range(0, len(heatmap_bychan)):
    ...
    ax=sns.heatmap(selected_rect_data, xticklabels=str, yticklabels=str, cmap='Blues',annot=selected_rect_data_raw, cbar_kws={'label': 'significance score'}, vmin=0, vmax=0.3)
    for text, show_annot in zip(ax.texts, (element for row in show_annot_array for element in row)):
        text.set_visible(show_annot)

    plt.title('Original F0, Site Number '+(np.array2string(channel_list[i])))
    plt.savefig(bin_folder + '\seabornheatmapnPSzola_29082022_chan'+np.array2string(channel_list[i])+'.png', dpi=500, bbox_inches='tight')
